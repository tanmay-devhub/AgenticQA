...
def _scan_run(run_dir: Path) -> RunSummary | None:
    ...
    # Try to guess the target from the workdir's target.* header. Python is
    # canonical (target.py); mutagen-qa-js writes target.js into the same runs/
    # tree, so we probe both without forcing a per-language branch elsewhere.
    for target_ext in ("py", "js", "ts"):
        target_file = run_dir / f"target.{target_ext}"
        if target_file.is_file():
            try:
                first_line = target_file.read_text(encoding="utf-8").splitlines()[0]
                target = first_line.lstrip('"# /').strip() or target_file.name
...
def create_app(
    ...
    *,
    ...
    def run_detail(request: Request, name: str) -> HTMLResponse:
        ...
        target_src = ""
        for ext in ("py", "js", "ts"):
            target_file = run_dir / f"target.{ext}"
            if target_file.is_file():
    ...
    def run_tests(name: str, round_idx: int) -> PlainTextResponse:
        run_dir = _resolve_run(name)
        # Probe .py first (canonical Python target), then .js / .ts for runs
        # produced by mutagen-qa-js. Same runs/ tree serves both.
        for ext in ("py", "js", "ts"):
            p = run_dir / f"test_round_{round_idx}.{ext}"
            if p.is_file():
