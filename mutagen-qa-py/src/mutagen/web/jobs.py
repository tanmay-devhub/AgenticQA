...
JobLanguage = Literal["python", "javascript"]

# Extension -> language classifier used when a repo target's suffix decides
# which pipeline runs. Kept small and explicit; new languages ship one at a
# time and each one adds a bin resolver + sidecar suffix below.
_LANG_BY_EXT: dict[str, JobLanguage] = {
    ".py": "python",
    ".js": "javascript",
    ".mjs": "javascript",
    ".jsx": "javascript",
...
# The JS CLI ships alongside the Python package at ``<repo>/mutagen-qa-js/bin/mutagen-js.js``.
# Override via MUTAGEN_JS_BIN when the layout changes (dev checkouts, packaging).
def _resolve_js_bin() -> Path | None:
    override = os.environ.get("MUTAGEN_JS_BIN")
    if override:
        p = Path(override)
        return p if p.is_file() else None
    # parents[0]=web, [1]=mutagen, [2]=src, [3]=mutagen-qa-py, [4]=repo root.
    candidate = Path(__file__).resolve().parents[4] / "mutagen-qa-js" / "bin" / "mutagen-js.js"
...
class Job:
    ...
    focus: str | None = None
    # Which pipeline to run. Paste-source jobs default from an explicit form
    # field; repo-mode jobs infer from the target file's extension.
...
class JobRegistry:
    ...
    def submit(
        ...
        *,
        ...
        if target_source is not None:
            ...
            # into ``workdir/target.<ext>`` (Windows raises on self-copy).
            sidecar_name = "_input.py" if language == "python" else "_input.js"
            (wd / sidecar_name).write_text(target_source, encoding="utf-8")
    ...
    def _prepare_repo_target(self, job: Job) -> Path:
        ...
        # Resolve target safely: must stay inside clone_dir, must exist, and
        # its extension must map to a language pipeline we know how to run.
        ...
        # Resolve symlinks BEFORE containment check; a repo symlink could
        # point outside the tree.
        real_candidate = candidate.resolve(strict=True)
        real_clone = clone_dir.resolve(strict=True)
        if real_clone not in real_candidate.parents and real_candidate != real_clone:
            raise RuntimeError(f"target path escapes repo root after symlink resolution: {rel!r}")
        lang = _LANG_BY_EXT.get(real_candidate.suffix.lower())
        if lang is None:
            raise RuntimeError(
                f"unsupported target extension {real_candidate.suffix!r}. "
                "Supported today: .py (Python), .js/.mjs/.jsx (JavaScript)."
            )
        # Sync the inferred language onto the job so _run dispatches correctly.
        job.language = lang
        sidecar_name = "_input.py" if lang == "python" else "_input.js"
        sidecar = job.workdir / sidecar_name
        sidecar.write_text(real_candidate.read_text(encoding="utf-8"), encoding="utf-8")

        # Pytest-side isolation: pin its rootdir + testpaths so the cloned
        # repo's own tests don't get auto-discovered during mutmut. Not needed
        # for JS since Stryker is pointed at explicit test files.
        if lang == "python":
        ...
        self._emit(job, {"type": "target_selected", "path": rel, "language": lang})
        return sidecar
    ...
    def _run(self, job: Job) -> None:
        ...
        try:
            if job.repo_url:
                ...
                sidecar_name = "_input.py" if job.language == "python" else "_input.js"
                target_path = job.workdir / sidecar_name

            # JS jobs are handled out-of-process by the mutagen-qa-js CLI.
            # It writes the same run.json shape to workdir/ so downstream
            # dashboard reads are uniform.
            if job.language == "javascript":
                self._run_js(job, target_path)
                return
    ...
    # -- JS pipeline dispatch ---------------------------------------------

    def _poll_js_progress(self, job: Job, stop_evt: threading.Event) -> None:
        """Emit ``round`` events by watching for ``round_N_report.json`` files.

        The JS CLI runs out-of-process and has no callback into our SSE stream,
        so we polled the workdir. Cheap: one directory listing every 2s.
        """
        seen: set[int] = set()
        while not stop_evt.wait(2.0):
            for report_file in sorted(job.workdir.glob("round_*_report.json")):
                try:
                    idx = int(report_file.stem.split("_")[1])
    ...
    def _run_js(self, job: Job, target_path: Path) -> None:
        js_bin = _resolve_js_bin()
        if js_bin is None:
            raise RuntimeError(
                "mutagen-qa-js CLI not found. Point MUTAGEN_JS_BIN at "
                "<repo>/mutagen-qa-js/bin/mutagen-js.js, or install it there."
            )
        cmd = [
            "node", str(js_bin),
            "run", str(target_path),
        ...
        # Spawn poller before subprocess so we don't miss round 1 events.
        stop_evt = threading.Event()
        poller = threading.Thread(
            target=self._poll_js_progress, args=(job, stop_evt),
            daemon=True, name=f"js-poll-{job.id[:8]}",
        )
        poller.start()
        ...
        if proc.returncode not in (0, 2):
            # Exit 2 is the JS CLI's "loop finished but pytest_ok=False on the
            # last round" signal -- still a real result with run.json written.
            # Anything else is a subprocess error.
            raise RuntimeError(
                f"mutagen-js exited {proc.returncode}: {(proc.stderr or '')[-800:]}"
            )

        # Load the run.json the JS CLI just wrote and mirror the terminal
        # event the Python path emits.
        run_json = job.workdir / "run.json"
        if not run_json.is_file():
            raise RuntimeError("JS run finished without writing run.json")
        try:
            data = json.loads(run_json.read_text(encoding="utf-8"))
        except json.JSONDecodeError as e:
            raise RuntimeError(f"JS run.json is malformed: {e}") from e

        job.final_kill_rate = data.get("final_kill_rate")
        job.status = "done"
        self._emit(job, {
            "type": "done",
            "final_kill_rate": job.final_kill_rate,
