...
**Python pipeline** of the mutagen-qa multi-language project. Point it at a
Python source file (or a GitHub repo URL) and it generates a pytest suite
optimized for **bug-catching power** (mutation kill rate), not test count.

The JavaScript sibling lives at [`../mutagen-qa-js/`](../mutagen-qa-js/).
Cross-language schemas live at [`../shared/`](../shared/). Design doc /
decision log at [`../CONTEXT.md`](../CONTEXT.md).
...
```bash
git clone <this-repo>
cd mutagen-qa/mutagen-qa-py
uv venv --python 3.12 .venv
uv pip install -e .
```

Put your keys in the repo-root `.env` (shared with the JS pipeline):
...
Python and JavaScript ship today. Each remaining language will ship
end-to-end before the next starts. All six candidates below have
production-grade mutation tools available. None get skipped:
...
tests/           195 tests for this tool itself
```

The FastAPI web dashboard also renders JavaScript runs from
[`../mutagen-qa-js/`](../mutagen-qa-js/) using the same code path -- both
languages share the `runs/` output shape and the cross-language schemas
under [`../shared/`](../shared/) (mutation-kind taxonomy + planner
technique hints).
...
pytest -q                          # 195 tests, ~15s on a warm cache
ruff check .
mypy src
```
...
**JavaScript pipeline shipped** in `mutagen-qa-js/`: T1/T2/T3 tiers,
classifier, planner, repair, debrief, Stryker mutation report, repo
mode with symlink-safe path resolution, and cross-language schema
enforcement. Multi-round loop end-to-end verified against Ollama Cloud
and Gemini; 65 unit tests + 1 admin-only skip.

Next: TypeScript / Java / C# / C++ pipelines.
