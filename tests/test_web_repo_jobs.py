...
@needs_git
def test_repo_mode_rejects_non_python_target(app_and_client):
    _app, c, tmp_path = app_and_client
    origin = _init_repo(tmp_path, {"web/index.js": "console.log('hi')\n"})
    r = c.post("/jobs", data={
    ...
    assert final["status"] == "failed"
    assert "Python" in (final["error"] or "") or ".js" in (final["error"] or "")
    # Language census still populated -- useful signal even on rejection.
    assert final["languages"].get("javascript") == 1
